...
def test_general(temporary_directory):
    ds = HetionetDataset(data_path=temporary_directory)
    edges_df = ds.get_edges()
    nodes_df = ds.get_nodes()

    hetero_data = ds.to_pytorch_geometric_heterodata()
    nodes_df.iloc[hetero_data["Cellular Component"].node_indices]
    edges_df.iloc[hetero_data[("Pharmacologic Class", "PCiC", "Compound")].edge_indices]
