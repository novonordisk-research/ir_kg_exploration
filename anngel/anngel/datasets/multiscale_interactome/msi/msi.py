...
class MSI:
    def __init__(
        ...
        self.protein2biological_function_file_path = (
            protein2biological_function_file_path
        )
        self.biological_function2biological_function_file_path = (
            biological_function2biological_function_file_path
        )
        ...
        self.biological_function2biological_function_directed = (
            biological_function2biological_function_directed
        )
    ...
    def load_graph(self):
        ...
        if (BIOLOGICAL_FUNCTION in self.nodes) and (
            PROTEIN_BIOLOGICAL_FUNCTION in self.edges
        ):
            self.components[
                "protein_to_biological_function"
            ] = ProteinToBiologicalFunction(
                self.protein2biological_function_directed,
                self.protein2biological_function_file_path,
            )
            self.add_edges(
                self.components["protein_to_biological_function"].edge_list,
                PROTEIN,
                BIOLOGICAL_FUNCTION,
            )

        if (BIOLOGICAL_FUNCTION in self.nodes) and (
            BIOLOGICAL_FUNCTION_BIOLOGICAL_FUNCTION in self.edges
        ):
            self.components[
                "biological_function_to_biological_function"
            ] = BiologicalFunctionToBiologicalFunction(
                self.biological_function2biological_function_directed,
                self.biological_function2biological_function_file_path,
            )
            self.add_edges(
                self.components["biological_function_to_biological_function"].edge_list,
                BIOLOGICAL_FUNCTION,
                BIOLOGICAL_FUNCTION,
            )
    ...
    def create_class_specific_adjacency_dictionary(self):
        ...
        for node in self.graph.nodes():
            ...
            for successor in successors:
                ...
                if (node_type == BIOLOGICAL_FUNCTION) and (
                    successor_type == BIOLOGICAL_FUNCTION
                ):
                    if successor in up_neighbors:
                        ...
                        self.add_to_cs_adj_dict(
                            node, DOWN_BIOLOGICAL_FUNCTION, successor
                        )
                    else:
                        assert False
                else:
        ...
        for from_node, adj_dict in self.cs_adj_dict.items():
            for node_type, to_nodes in adj_dict.items():
                ...
                for to_node in to_nodes:
                    self.graph[from_node][to_node][WEIGHT] = weights[node_type] / float(
                        num_typed_nodes
                    )
